...
def getMovieDetails(movieid):
    query = (
        '{"jsonrpc": "2.0", "method": "VideoLibrary.GetMovieDetails", "params": {"movieid": %s, "properties" : ["title", "file"]}, "id": 1}'
        % movieid
    )
...
def GetTVShowEpisodes(showId):
    query = (
        '{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodes", "params": {"properties": ["showtitle", "thumbnail", "file"], "sort": {"method": "episode"}, "tvshowid":%s}, "id": 1}'
        % showId
    )
...
def getEpisodeDetails(episodeId):
    query = (
        '{"jsonrpc": "2.0", "method": "VideoLibrary.GetEpisodeDetails", "params": {"episodeid": %s, "properties" : ["showtitle", "file"]}, "id": 1}'
        % episodeId
    )
