...
class SubFinder:
    ...
    def getSRT(self, fileLoc):
        ...
        for fname in os.listdir(os.path.dirname(fileLoc)):
            ...
            if (
                len(parts) == 2
                and parts[1].lower() == ".mpbak"
                and parts[0].endswith(".srt")
            ):
                originalname = os.path.join(
                    os.path.dirname(fileLoc), fname[: -len(".mpbak")]
                )
                ...
                xbmcvfs.rename(
                    os.path.join(os.path.dirname(fileLoc), fname), originalname
                )
                return originalname
        ...
        elif (ext.lower() == ".m4v" or ext.lower() == ".mp4") and self.Addon.getSetting(
            "usemp4box"
        ) == "true":
