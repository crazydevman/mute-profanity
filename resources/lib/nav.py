...
def get_severities():
    ...
    severities = {
        "Expletives": mapping[Addon.getSetting("blockcat1")],
    ...
    for name, tuples in categories.iteritems():
        ...
        for word, severity in tuples:
            if show_all or filter.is_blocked(severity, name, severities):
                params = {"mode": "word-details", "wordId": wordId, "categoryId": catId}
...
def handle(params):
    ...
    if mode == "mute-movie":
        ...
        if blockNum:
            line2 = (
                "Current category setting blocks words with severity >= %d" % blockNum
            )
        else:
